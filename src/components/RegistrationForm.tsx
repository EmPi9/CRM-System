...
export default function RegistrationForm(){
    ...
    const handleSubmit = async (values: { 
      ...
      try {
        ...
        if (error.response.status == 409) {
            openNotification('Конфликт', 'Такой пользователь уже есть. Исправте данные')
            return
        }
        openNotification('Ошибка сервера', 'Ошибка работы сервера, пожалуйста попробуйте позже')
        return          
      }
    }  
    ...
    return (
        ...
        :
        <Form style={{ width: 500 }} layout="vertical" onFinish={handleSubmit} form={form}>
